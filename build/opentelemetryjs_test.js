    ...
    const onClick = () => {
        ...
        context.with(trace.setSpan(context.active(), singleSpan), () => {
            getData(url).then((_data) => {
                var _a;
                (_a = trace.getSpan(context.active())) === null || _a === void 0 ? void 0 : _a.addEvent('fetching-single-span-completed');
                singleSpan.end();
            });
        });
        for (let i = 0, j = 5; i < j; i += 1) {
            ...
            context.with(trace.setSpan(context.active(), span), () => {
                getData(url).then((_data) => {
                    var _a;
                    (_a = trace.getSpan(context.active())) === null || _a === void 0 ? void 0 : _a.addEvent(`fetching-span-${i}-completed`);
                    span.end();
                });
            });
        }
